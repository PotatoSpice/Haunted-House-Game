...
public class OrderedBinaryTreeAVL<T> extends LinkedBinaryTree<T> implements BinarySearchTreeADT<T> {
    ...
    public OrderedBinaryTreeAVL(Object t){
        super((T)t);
    }

    private int height(AVLBinaryTreeNode node ) {  return node == null ? -1 : node.height; }
    private int balance(AVLBinaryTreeNode<T> node){ return node.left.height - node.right.height;}
    private int maxHeight(int height1, int height2) { return height1>height2 ? height1:height2; }
